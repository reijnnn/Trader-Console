...
def create_app(config=None, mode=None):
    app = Flask(__name__)
    ...
    logger.info('"{}" application started with config "{}" in "{}" mode'.format(app.config['APP_NAME'], config, mode))
...
def configure_logging(app):
    error_log_handler = RotatingFileHandler(app.config['APP_LOG_ERROR_FILE'], maxBytes=app.config['APP_LOG_MAX_BYTES'],
                                            backupCount=app.config['APP_LOG_BACKUP_COUNT'])
    error_log_handler.setFormatter(
        logging.Formatter(app.config['APP_LOG_MSG_FORMAT'], app.config['APP_LOG_DATETIME_FORMAT']))
    error_log_handler.setLevel(logging.ERROR)
    logger.addHandler(error_log_handler)

    info_log_handler = RotatingFileHandler(app.config['APP_LOG_INFO_FILE'], maxBytes=app.config['APP_LOG_MAX_BYTES'],
                                           backupCount=app.config['APP_LOG_BACKUP_COUNT'])
    ...
    debug_log_handler = RotatingFileHandler(app.config['APP_LOG_DEBUG_FILE'], maxBytes=app.config['APP_LOG_MAX_BYTES'],
                                            backupCount=app.config['APP_LOG_BACKUP_COUNT'])
