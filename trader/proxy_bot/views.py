...
from ..extensions       import login_manager, csrf, db, proxy_bot, logger
...
def monitor_proxy_bot(page):
	...
	return render_template('proxy_bot.html', proxies=proxies, pagination=pagination, status=proxy_bot.get_status(), thread_status=proxy_bot.get_thread_status(), active_proxy_id=proxy_bot.get_active_proxy_id())
...
def add_proxy():
	if request.method == 'POST':
		...
		for proxy in proxy_list:
			...
			exist_proxy = db.session.query(Proxies).filter_by(proxy_ip=type_ip_port[1], proxy_port=type_ip_port[2]).count()
			if exist_proxy:
		...
		if len(exists_proxy_list):
			flash("{} proxy server/servers already exist: {}".format(len(exists_proxy_list), " ".join(exists_proxy_list)), 'warning')
