...
class Proxies(db.Model):
	...
	def __repr__(self):
		return "<Proxy(proxy_id='%s', proxy_type='%s', proxy_ip='%s', proxy_port='%s', proxy_weight='%s', proxy_good_try='%s', proxy_bad_try='%s')>" % (
						self.proxy_id, self.proxy_type, self.proxy_ip, self.proxy_port, self.proxy_weight, self.proxy_good_try, self.proxy_bad_try)
