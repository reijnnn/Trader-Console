from ..extensions           import logger, db, proxy_bot
...
class Telegram_bot(threading.Thread):
	...
	def cancel(self):
		...
		logger.info("Telegram bot stoped")
			...
			while self.is_active:
				...
				try:
					...
					for update in updates['result']:
						...
						if update['message']['text'] == '/help':
							self.send_message({'chat_id': chat_id
											, 'text': "Available commands:" + "\n" +
														"/id - to register in the system, send command's result to admin" + "\n" +
						...
						task = Task(update['message']['text'], chat_id, reply_to_message_id)
	...
	def get_me(self):
		...
		except (Timeout, ProxyError, SSLError) as e:
