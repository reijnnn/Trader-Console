...
class Notifications(db.Model):
    ...
    def __repr__(self):
        return "<Notification(notif_id='%s', notif_status='%s', notif_date='%s', notif_text='%s', " \
               "chat_id='%s', task_id='%s')>" % (
                self.notif_id, self.notif_status, self.notif_date, self.notif_text,
                self.chat_id, self.task_id)
