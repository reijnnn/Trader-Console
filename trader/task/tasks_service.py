...
def complete_task(task_id):
    db.session.query(Tasks).filter_by(task_id=task_id).update(dict(task_status=TaskStatus.COMPLETED))
    db.session.commit()


def cancel_task(task_id):
    db.session.query(Tasks).filter_by(task_id=task_id).update(dict(task_status=TaskStatus.CANCELED))
    db.session.commit()


def drop_task(task_id):
    db.session.query(Tasks).filter_by(task_id=task_id).update(dict(task_status=TaskStatus.DROPPED))
    db.session.commit()
...
def update_task_exec_time(task_id):
    db.session.query(Tasks).filter_by(task_id=task_id).update(
        dict(last_exec_time=datetime.now().replace(microsecond=0)))
    db.session.commit()
...
def end_task(task_id):
    db.session.query(Tasks).filter_by(task_id=task_id).update(dict(task_status=TaskStatus.ENDED))
    db.session.commit()
