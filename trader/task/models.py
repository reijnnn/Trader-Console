...
class Tasks(db.Model):
    ...
    def __repr__(self):
        return "<Task(task_id='%s', task_name='%s', task_params='%s', task_status='%s', " \
               "task_date='%s', chat_id='%s', task_cnt_errors='%s')>" % (
                self.task_id, self.task_name, self.task_params, self.task_status,
                self.task_date, self.chat_id, self.task_cnt_errors)
