...
from ..trader_bot.strategies      import Strategies, Check_functions
...
class Task():
   ...
   def cancel(self):
      ...
      db.session.query(Tasks).filter_by(task_id=task_id, chat_id=self.chat_id).update(dict(task_status=Task_status.CANCELED))
      db.session.commit()
   ...
   def list(self, strategy=None):
      query = db.session.query(Tasks).filter_by(task_status=Task_status.ACTIVE, chat_id=self.chat_id)
      if strategy:
   ...
   def help(self, strategy=None):
      if strategy:
         ...
         self.add_task_notification(text=
            'Command examples:'                         + "\n"   +
   ...
   def parse_params(self, params):
      ...
      for param_key, param_val in params.items():
         if param_key not in self.config['strategies'][self.strategy]['params']:
            ...
            error_stack.append('In pair "{}={}" [key=val] one of the elements is empty'.format(param_key, param_val))
      return error_stack
   ...
   def call_check_functions(self, params):
      ...
      for param_key, param_val in params.items():
         if 'check_function' in self.config['strategies'][self.strategy]['params'][param_key]:
            ...
            if not getattr(Check_functions, function_name)(param_val):
